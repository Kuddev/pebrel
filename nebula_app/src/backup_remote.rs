...
impl BackupRemoteConfig {
    ...
    pub fn save(&self) -> Result<(), String> {
        ...
        std::fs::write(config_path(), text).map_err(|err| {
            tr_args("backup_remote.config_write_failed", &[("error", &err.to_string())])
        })
    }
}
    ...
    match cfg.protocol {
        ...
        BackupProtocol::WebDav => {
            ...
            if !url.starts_with("https://") && !cfg.allow_http {
                return Err(tr("backup_remote.webdav_https_required").to_owned());
            }
            let username = cfg.webdav_username.trim().to_owned();
            if username.is_empty() {
                return Err(tr("backup_remote.webdav_username_missing").to_owned());
            }
            let password = webdav_password().ok_or(tr("backup_remote.webdav_password_missing"))?;
        ...
        BackupProtocol::S3 => {
            ...
            if !endpoint.starts_with("https://") && !cfg.allow_http {
                return Err(tr("backup_remote.s3_https_required").to_owned());
            }
            ...
            let secret_key = s3_secret().ok_or(tr("backup_remote.s3_secret_missing"))?;
        ...
        BackupProtocol::Sftp => {
            ...
            if destination.is_empty() {
                return Err(tr("backup_remote.sftp_destination_missing").to_owned());
            }
...
pub fn push(packet: &[u8]) -> Result<String, String> {
    ...
    match prune(backend.as_ref()) {
        Ok(kept) if kept > 1 => message
            .push_str(&tr_args("backup_remote.retention_suffix", &[("kept", &kept.to_string())])),
        Ok(_) => {},
        Err(err) => {
...
impl Backend for FolderBackend {
    fn put(&self, name: &str, bytes: &[u8]) -> Result<(), String> {
        std::fs::create_dir_all(&self.root).map_err(|err| {
            tr_args("backup_remote.folder_create_failed", &[("error", &err.to_string())])
        })?;
        crate::atomic_file::write(&self.root.join(name), bytes).map_err(|err| {
            tr_args("backup_remote.write_backup_failed", &[("error", &err.to_string())])
        })
    }

    fn get(&self, name: &str) -> Result<Vec<u8>, String> {
        std::fs::read(self.root.join(name)).map_err(|err| {
            tr_args("backup_remote.read_backup_failed", &[("error", &err.to_string())])
        })
    }
    ...
    fn list(&self) -> Result<Vec<String>, String> {
        let entries = match std::fs::read_dir(&self.root) {
            ...
            Err(err) => {
                return Err(tr_args(
                    "backup_remote.folder_list_failed",
                    &[("error", &err.to_string())],
                ));
            },
        };
        ...
        for entry in entries {
            let entry = entry.map_err(|err| {
                tr_args("backup_remote.folder_list_failed", &[("error", &err.to_string())])
            })?;
    ...
    fn delete(&self, name: &str) -> Result<(), String> {
        std::fs::remove_file(self.root.join(name)).map_err(|err| {
            tr_args("backup_remote.delete_old_failed", &[("error", &err.to_string())])
        })
    }
...
impl WebDavBackend {
    ...
    fn put_once(&self, name: &str, bytes: &[u8]) -> Result<u16, String> {
        let response = http_agent()
            ...
            .send(bytes)
            .map_err(|err| {
                tr_args("backup_remote.upload_failed", &[("error", &err.to_string())])
            })?;
    ...
    fn mkcol(&self) -> Result<(), String> {
        let request = ureq::http::Request::builder()
            ...
            .body(Vec::new())
            .map_err(|err| {
                tr_args("backup_remote.mkcol_build_failed", &[("error", &err.to_string())])
            })?;
        let response = http_agent().run(request).map_err(|err| {
            tr_args("backup_remote.remote_directory_create_failed", &[("error", &err.to_string())])
        })?;
        match response.status().as_u16() {
            ...
            status => Err(tr_args(
                "backup_remote.remote_directory_http_failed",
...
impl Backend for WebDavBackend {
    fn put(&self, name: &str, bytes: &[u8]) -> Result<(), String> {
        ...
        match status {
            ...
            401 | 403 => Err(tr("backup_remote.webdav_auth_failed").to_owned()),
            other => {
                Err(tr_args("backup_remote.upload_http_failed", &[("status", &other.to_string())]))
            },
        }
    }
    ...
    fn get(&self, name: &str) -> Result<Vec<u8>, String> {
        let mut response = http_agent()
            ...
            .call()
            .map_err(|err| {
                tr_args("backup_remote.download_failed", &[("error", &err.to_string())])
            })?;
        match response.status().as_u16() {
            200 => response.body_mut().read_to_vec().map_err(|err| {
                tr_args("backup_remote.remote_read_failed", &[("error", &err.to_string())])
            }),
            401 | 403 => Err(tr("backup_remote.webdav_auth_failed").to_owned()),
            status => Err(tr_args(
                "backup_remote.download_http_failed",
                &[("status", &status.to_string())],
            )),
        }
    }
    ...
    fn list(&self) -> Result<Vec<String>, String> {
        ...
        let mut response = http_agent().run(request).map_err(|err| {
            tr_args("backup_remote.remote_list_failed", &[("error", &err.to_string())])
        })?;
        match response.status().as_u16() {
            207 | 200 => {},
            // 目录还不存在 = 还没有任何备份。
            ...
            status => {
                return Err(tr_args(
                    "backup_remote.remote_list_http_failed",
                    &[("status", &status.to_string())],
                ));
            },
        }
        let body = response.body_mut().read_to_vec().map_err(|err| {
            tr_args("backup_remote.list_read_failed", &[("error", &err.to_string())])
        })?;
    ...
    fn delete(&self, name: &str) -> Result<(), String> {
        let request = ureq::http::Request::builder()
            ...
            .body(Vec::new())
            .map_err(|err| {
                tr_args("backup_remote.delete_request_build_failed", &[("error", &err.to_string())])
            })?;
        let response = http_agent().run(request).map_err(|err| {
            tr_args("backup_remote.delete_old_failed", &[("error", &err.to_string())])
        })?;
        match response.status().as_u16() {
            200 | 204 | 404 => Ok(()),
            status => Err(tr_args(
                "backup_remote.delete_old_http_failed",
                &[("status", &status.to_string())],
            )),
        }
    }
...
impl S3Backend {
    ...
    fn request(
        ...
        let request = ureq::http::Request::builder()
            ...
            .body(body.to_vec())
            .map_err(|err| {
                tr_args("backup_remote.s3_request_build_failed", &[("error", &err.to_string())])
            })?;
        let mut response = http_agent().run(request).map_err(|err| {
            tr_args("backup_remote.connection_failed", &[("error", &err.to_string())])
        })?;
        let status = response.status().as_u16();
        let bytes = response.body_mut().read_to_vec().map_err(|err| {
            tr_args("backup_remote.response_read_failed", &[("error", &err.to_string())])
        })?;
        Ok((status, bytes))
    }
...
impl Backend for S3Backend {
    fn put(&self, name: &str, bytes: &[u8]) -> Result<(), String> {
        ...
        match status {
            ...
            _ => Err(tr_args(
                "backup_remote.upload_failed",
                &[("error", &Self::explain(status, &body))],
            )),
        }
    }
    ...
    fn list(&self) -> Result<Vec<String>, String> {
        ...
        for prefix in [ARCHIVE_PREFIX, LEGACY_ARCHIVE_PREFIX] {
            ...
            match status {
                ...
                _ => {
                    return Err(tr_args(
                        "backup_remote.remote_list_failed",
                        &[("error", &Self::explain(status, &body))],
                    ));
                },
            }
    ...
    fn delete(&self, name: &str) -> Result<(), String> {
        ...
        match status {
            200 | 204 => Ok(()),
            _ => Err(tr_args(
                "backup_remote.delete_old_failed",
                &[("error", &Self::explain(status, &body))],
            )),
        }
    }
...
impl SftpBackend {
    ...
    {
        let runtime = crate::ssh_session::runtime().map_err(|err| {
            tr_args("backup_remote.ssh_runtime_unavailable", &[("error", &err.to_string())])
        })?;
        let destination = self.destination.clone();
        runtime.block_on(async move {
            let sftp = crate::ssh_session::open_sftp(&destination).await.map_err(|err| {
                tr_args("backup_remote.sftp_connection_failed", &[("error", &err.to_string())])
            })?;
            job(sftp).await
        })
    }
...
impl Backend for SftpBackend {
    fn put(&self, name: &str, bytes: &[u8]) -> Result<(), String> {
        ...
        self.run(|sftp| async move {
            ...
            let mut file = sftp
                ...
                .await
                .map_err(|err| {
                    tr_args("backup_remote.remote_file_open_failed", &[("error", &err.to_string())])
                })?;
            file.write_all(&bytes).await.map_err(|err| {
                tr_args("backup_remote.write_failed", &[("error", &err.to_string())])
            })?;
            file.shutdown().await.map_err(|err| {
                tr_args("backup_remote.write_finish_failed", &[("error", &err.to_string())])
            })?;
            Ok(())
        })
    }
    ...
    fn get(&self, name: &str) -> Result<Vec<u8>, String> {
        ...
        let remote = self.remote_file(name);
        self.run(|sftp| async move {
            let mut file = sftp.open(remote).await.map_err(|err| {
                tr_args("backup_remote.remote_file_open_failed", &[("error", &err.to_string())])
            })?;
            let mut bytes = Vec::new();
            file.read_to_end(&mut bytes).await.map_err(|err| {
                tr_args("backup_remote.read_failed", &[("error", &err.to_string())])
            })?;
            Ok(bytes)
        })
    }
    ...
    fn delete(&self, name: &str) -> Result<(), String> {
        let remote = self.remote_file(name);
        self.run(|sftp| async move {
            sftp.remove_file(remote).await.map_err(|err| {
                tr_args("backup_remote.delete_old_failed", &[("error", &err.to_string())])
            })
        })
    }
