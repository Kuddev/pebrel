...
pub(super) fn pick_private_key_file(owner: &Window) -> Option<Result<PathBuf, String>> {
    let path =
        platform::pick_file(owner, &t!("file_dialog.choose_private_key"), PRIVATE_KEY_FILTERS)?;
...
pub(crate) fn pick_private_key_file_unowned() -> Option<Result<PathBuf, String>> {
    let path =
        platform::pick_file_unowned(&t!("file_dialog.choose_private_key"), PRIVATE_KEY_FILTERS)?;
    Some(validate_private_key_path(&path))
}
...
pub(crate) fn pick_private_key_file_with_hwnd(
    ...
    let path = platform::pick_file_with_hwnd(
        hwnd,
        &t!("file_dialog.choose_private_key"),
        PRIVATE_KEY_FILTERS,
    )?;
    Some(validate_private_key_path(&path))
}
...
pub(crate) fn pick_upload_files_with_hwnd(
    ...
    platform::pick_files_with_hwnd(
        hwnd,
        &t!("file_dialog.choose_upload_files"),
        &[ALL_FILES_FILTER],
    )
}
...
pub(crate) fn validate_private_key_path(path: &std::path::Path) -> Result<PathBuf, String> {
    let contents = std::fs::read(path).map_err(|err| {
        t!("file_dialog.private_key.read_failed", path = path.display(), error = err).to_string()
    })?;
...
pub(super) fn save_backup_file(owner: &Window) -> Option<PathBuf> {
    platform::save_file(
        owner,
        &t!("file_dialog.export_backup"),
        BACKUP_FILTERS,
        "nebula-backup.nebula-backup",
    )
}
