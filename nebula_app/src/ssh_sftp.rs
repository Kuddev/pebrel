...
impl SftpController {
    ...
    pub(crate) fn copy_from(
        ...
        let path = snapshot.path;
        let progress =
            TransferProgress::new(t!("ssh.sftp.copying", name = &entry.name).to_string(), entry.size.saturating_mul(2));
        self.start_job(SftpPhase::Working, Some(progress), move |context| async move {
    ...
    pub(crate) async fn list_dir(&self, path: &str) -> Result<Vec<SftpEntry>, String> {
        ...
        if slot.is_none() {
            *slot = Some(
                crate::ssh_session::open_sftp(&self.destination)
                    .await
                    .map_err(|err| t!("ssh.sftp.connection_failed", destination = &self.destination, error = err).to_string())?,
            );
        }
        ...
        let result =
            read_remote_dir(slot.as_ref().expect("SFTP browser session initialized"), &path)
                .await
                .map_err(|err| t!("ssh.sftp.read_directory_failed", path = path, error = err).to_string());
        if result.is_err() {
