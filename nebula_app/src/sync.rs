...
impl SyncConfig {
    ...
    pub fn save(&self) -> Result<(), String> {
        ...
        std::fs::write(config_path(), text)
            .map_err(|err| tr_args("sync.config_write_failed", &[("error", &err.to_string())]))
    }
}
...
fn payload_from_json(bytes: &[u8]) -> Result<SyncPayload, String> {
    let value: serde_json::Value = serde_json::from_slice(bytes)
        .map_err(|err| tr_args("sync.payload_corrupt", &[("error", &err.to_string())]))?;
    ...
    match response.status().as_u16() {
        200 => {
            ...
            let body = response.body_mut().read_to_vec().map_err(|err| {
                tr_args("sync.remote_read_failed", &[("error", &err.to_string())])
            })?;
...
fn put_remote(
    ...
    let response = request
        .send(packet)
        .map_err(|err| tr_args("sync.upload_failed", &[("error", &err.to_string())]))?;
    match response.status().as_u16() {
        200 | 201 | 204 => Ok(true),
        412 => Ok(false),
        401 | 403 => Err(tr("sync.webdav_auth_failed").to_owned()),
...
fn preflight() -> Result<(SyncConfig, String, String), String> {
    ...
    let password = webdav_password().ok_or(tr("sync.webdav_password_missing"))?;
    let passphrase = sync_passphrase().ok_or(tr("sync.passphrase_missing"))?;
...
pub fn push() -> Result<SyncOutcome, String> {
    ...
    if put_remote(&cfg, &password, &packet, fresh.etag.as_deref())? {
        Ok(SyncOutcome { message: tr("sync.push_merged").to_owned(), history_changed })
    } else {
...
pub fn pull() -> Result<SyncOutcome, String> {
    ...
    if !settings_before && !history_changed {
        return Ok(SyncOutcome { message: tr("sync.up_to_date").to_owned(), history_changed });
    }
    Ok(SyncOutcome {
        message: tr_args(
            "sync.pull_success",
