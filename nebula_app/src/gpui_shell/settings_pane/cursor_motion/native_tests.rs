...
#[test]
#[ignore = "requires a native desktop and an isolated PEBREL_CURSOR_QA_DIR/config"]
fn native_cursor_motion_menu_keeps_all_choices_visible() {
    ...
    let theme = std::env::var("PEBREL_CURSOR_QA_THEME").unwrap_or_else(|_| "Nord".into());
    let theme = ThemeName::from_prompt_name(&theme).expect("built-in theme");
    let chinese = std::env::var("PEBREL_CURSOR_QA_LANGUAGE").is_ok_and(|value| value == "zh-CN");
            ...
            cx.spawn(async move |cx| {
                ...
                let opened = cx
                    .update_window(handle.into(), |_, window, cx| -> Result<(), String> {
                        ...
                        if view.runtime.cursor_motion != nebula_settings::CursorMotion::Smooth
                            || view.runtime.ai_toasts
                        {
                            return Err(
                                "the motion and notification switch are not independently loaded"
                                    .into(),
                            );
                        }
                        let select =
                            view.select_of("cursor_motion").ok_or("motion control missing")?;
                        if select.read(cx).selected_index(cx).map(|path| path.row) != Some(1) {
                            return Err(
                                "motion selection does not reflect the persisted value".into()
                            );
                        }
                        let focus = select.read(cx).focus_handle(cx);
                        // The Cursor group follows Terminal appearance. Scroll the real settings
                ...
                if opened.is_ok() {
                    ...
                    std::fs::write(
                        ...
                        serde_json::to_vec(&serde_json::json!({
                            ...
                            "selected_mode": "smooth", "ai_toasts_enabled": false,
                        }))
                        .unwrap(),
                    )
                    .unwrap();
