...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if wants_gpui_shell(&options) {
        ...
        gpui_shell::run_shell(initial_cwd, shell_id);
        return Ok(());
    }
...
fn try_hand_over_to_resident(options: &Options) -> bool {
    ...
    // `--shell` 必须跟着交接一起走：驻留实例那边默认只认目录，漏掉它就会把
    // 「用 Ubuntu 打开」变成又一个 PowerShell 标签，而且毫无提示。
    let shell_id = terminal_options.shell_id();
    let launch_dir = terminal_options.resolved_working_directory().filter(|path| path.is_dir());
    if !options.daemon
        && !has_command
    ...
    let plain_launch = !options.daemon && terminal_options.working_directory.is_none() && !has_command;
