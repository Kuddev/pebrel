        ...
        match (node, range) {
            (_, Some(range)) if image.is_some() => {
                ...
                let alt = if alt.is_empty() {
                    current_format(Message::AnswerImageDefaultAlt, &[("index", &(image_count + 1).to_string())])
                } else {
                    alt.to_owned()
                };
...
pub fn local_image_path(target: &str, base: &Path) -> Result<PathBuf, String> {
    ...
    let root = base.canonicalize().map_err(|_| current_text(Message::AnswerUnknownDir).to_owned())?;
    let path = path.canonicalize().map_err(|_| current_text(Message::AnswerMissingFile).to_owned())?;
...
pub fn read_image(path: &Path) -> Result<Vec<u8>, String> {
    ...
    let file = std::fs::File::open(path).map_err(|_| current_text(Message::AnswerOpenFailed).to_owned())?;
    let metadata = file.metadata().map_err(|_| current_text(Message::AnswerMetadataFailed).to_owned())?;
