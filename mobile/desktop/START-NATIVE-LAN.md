# Pebrel 0.4.8 手机连接测试 / Mobile connection preview
...
此次局域网继续兼容 Android 0.4.3；请覆盖安装配套 0.4.8 APK 测试流式画面与恢复连接。
...
新 v2 链路使用端到端加密，建议配套 0.4.8 手机构建；旧 0.4.3 不支持。
...
离线包 Pebrel-Relay-manual-0.4.6.tar.gz 沿用已核验服务程序，与 0.4.8 客户端兼容，
...
the paired 0.4.8 build unifies compact/direct input and the optional editor for PC
...
remain compatible with 0.4.8 clients. This does not guarantee installation on
