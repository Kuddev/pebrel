    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        try:
            with sync_playwright() as playwright:
                ...
                # Search may match a complete guide or a section within it.
                expect(page).to_have_url(__import__('re').compile(r'/pebrel/[^#?]+/index\.html(?:#.*)?$'))
                expect(page.locator('#article-content h2').first).to_be_visible()
                page.keyboard.press('Control+k')
                page.locator('#search-input').fill('字体')
                ...
                page.reload()
                expect(page.locator('html')).to_have_attribute('data-theme', 'dark')
                dark_background = page.locator('.code-block').first.evaluate('el => getComputedStyle(el).backgroundColor')
                assert all(int(channel) < 128 for channel in __import__('re').findall(r'\d+', dark_background)[:3])
