...
点击左上角工具栏的 **打开项目文件夹…**，侧边标签与顶部标签布局均可使用。
也可以用 Tab 聚焦该按钮，再按 Enter 打开。选择文件夹后，Pebrel 会使用默认
Shell 在该目录新建终端标签；所选目录仅对新标签优先于 Profile 中的启动目录，
不会向现有终端发送命令，也不会改动已保存的 Profile。取消选择不会新建标签；
目录已不存在或不可访问时会显示提醒，可重新选择。
可查看[深色侧边布局与浅色顶部布局的工具栏示例](docs/screenshots/project-folder-entry.png)。
...
对所有新显示或刷新的应用内卡片生效。隐藏卡片不会同意或拒绝请求，也不会改变
