...
complete -c pebrel -n "__fish_pebrel_needs_command" -l socket -d 'Path for IPC socket creation' -r -F
...
complete -c pebrel -n "__fish_pebrel_using_subcommand help; and not __fish_seen_subcommand_from ctl env window tab pane agent mobile-bridge migrate config help" -f -a "ctl" -d 'Agent-oriented terminal control: split panes, run commands, start Codex/Claude, send prompts, wait for state changes, and read verified terminal output'
complete -c pebrel -n "__fish_pebrel_using_subcommand help; and not __fish_seen_subcommand_from ctl env window tab pane agent mobile-bridge migrate config help" -f -a "env" -d 'Report this pane\'s terminal identity, the control-plane path, and every command available to it. Answers even with no runtime reachable, so an agent can always discover what it has instead of guessing'
complete -c pebrel -n "__fish_pebrel_using_subcommand help; and not __fish_seen_subcommand_from ctl env window tab pane agent mobile-bridge migrate config help" -f -a "window" -d 'Control terminal windows'
complete -c pebrel -n "__fish_pebrel_using_subcommand help; and not __fish_seen_subcommand_from ctl env window tab pane agent mobile-bridge migrate config help" -f -a "tab" -d 'Control tabs within one terminal window'
complete -c pebrel -n "__fish_pebrel_using_subcommand help; and not __fish_seen_subcommand_from ctl env window tab pane agent mobile-bridge migrate config help" -f -a "pane" -d 'Inspect and drive terminal panes: list, read, send, paste, wait, and change layout'
complete -c pebrel -n "__fish_pebrel_using_subcommand help; and not __fish_seen_subcommand_from ctl env window tab pane agent mobile-bridge migrate config help" -f -a "agent" -d 'Inspect and drive the AI agents running in panes: list, send, read, wait'
complete -c pebrel -n "__fish_pebrel_using_subcommand help; and not __fish_seen_subcommand_from ctl env window tab pane agent mobile-bridge migrate config help" -f -a "mobile-bridge" -d 'Serve the bounded mobile control protocol over an authenticated SSH exec channel. Does not open a network listener or print the local runtime token'
complete -c pebrel -n "__fish_pebrel_using_subcommand help; and not __fish_seen_subcommand_from ctl env window tab pane agent mobile-bridge migrate config help" -f -a "migrate" -d 'Migrate the configuration file'
complete -c pebrel -n "__fish_pebrel_using_subcommand help; and not __fish_seen_subcommand_from ctl env window tab pane agent mobile-bridge migrate config help" -f -a "config" -d 'Validate or create the Pebrel configuration'
complete -c pebrel -n "__fish_pebrel_using_subcommand help; and not __fish_seen_subcommand_from ctl env window tab pane agent mobile-bridge migrate config help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
