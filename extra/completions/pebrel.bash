_pebrel() {
    ...
    case "${cmd}" in
        pebrel)
            opts="-q -v -e -T -o -h -V --print-events --ref-test --embed --gpui --config-file --socket --daemon --working-directory --hold --command --title --class --option --help --version ctl env window tab pane agent mobile-bridge migrate config help"
        ...
        pebrel__env)
            ...
            ;;
        pebrel__help)
            opts="ctl env window tab pane agent mobile-bridge migrate config help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
